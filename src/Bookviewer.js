...
import { Grid, Typography,Button, Rating , useMediaQuery} from '@mui/material';
...
const Bookviewer = () => {
    ...
    return (
		<div>
            ...
            <Grid container lg={12} xs={12} style={{background:'#F8F5F0'}}>
				<Grid container lg={10} xs={12} sx={{ margin:'auto',justifyContent:'space-between',flexDirection:mobile?"column-reverse":"" }}>
					
					<Grid container lg={5} xs={11} sx={{justifyContent:mobile?'center':""}}> 
					{mobile?"":	<Grid item lg={12}>
						...
						{mobile ? "" : <Grid item lg={2} style={{ display: 'flex', alignItems: 'center' }}>
							<Chip label={books.category} variant="outlined" style={{ background: '#DCF4F9', fontSize: "14px", color: '#1B1D1C', fontFamily: 'Space Grotesk' }} />
						</Grid>}
					{mobile?"":	<Grid item lg={2}>
						<span style={{color: '#206A8F',fontFamily: "Space Grotesk",fontSize: '16px',fontWeight: 700,
	lineHeight: 'normal',alignItems:'center',display:'flex',marginTop:'6px',
	letterSpacing: '0.48px'}}>
	<StarIcon sx={{ color: '#FFD700', marginRight: '5px' }} />{books.averageRating}</span>
						</Grid>}
						...
						<Grid item lg={10} xs={11} sx={{display:'flex',justifyContent:mobile?"space-between":'space-between',marginTop:'10px',alignItems:'center'}}>
								  <Grid item lg={3} xs={4}>
										  <img src={books.pdfPath} style={{ width: "100px", height: "100px",borderRadius:'50%' }} />
								  </Grid>
									  <Grid item lg={8} xs={7} sx={{ alignItems: 'center', display: 'flex' }}>
										  <Grid item lg={12}>
					...
					<Grid container lg={5} >
						{mobile ? <Grid container xs={12} sx={{justifyContent:'center'}}>
						<Grid item xs={10} sx={{justifyContent:'end',display:'flex'}}>
							<span style={{
								color: '#206A8F', fontFamily: "Space Grotesk", fontSize: '16px', fontWeight: 700,
								lineHeight: 'normal', alignItems: 'center', display: 'flex', marginTop: '6px',
								letterSpacing: '0.48px'
							}}>
								<StarIcon sx={{ color: '#FFD700', marginRight: '5px' }} />{books.averageRating}</span>
							...
							<Typography variant="h6" sx={{color:'#005B9D',fontSize:'26px',fontWeight:'700',fontFamily:'Space Grotesk',textAlign:'start',marginTop:'20px'}}>
								{books.title}
							</Typography>
							</Grid>
							</Grid>: ""}
					<Grid item lg={12} xs={12} className="book-container" style={{display:'flex',alignItems:'center',justifyContent:'center',marginTop:mobile?"24px":'94px'}}>
                        <img
                            src={books.coverImagePath}
                            alt={books.title}
                            style={{
                                width:mobile?"156px":'310px',height:mobile?"208px":'385px',
                                objectFit: 'cover',
                                borderRadius: '12px'
                            }}
							/>
						...
						<Grid container lg={12} xs={12} sx={{ justifyContent: 'center', display: 'flex' }}>
							<Grid item lg={12} xs={12} style={{justifyContent:'center',display:'flex'}}>
							<Button onClick={handleClick1} disabled={books.bookReviewCount===0} sx={{
  fontSize: '18px',
  fontWeight: '700',
  lineHeight: '32px',
  textTransform: 'none',
  ...
  '&:hover': {
	  ...
	  backgroundColor:"#005B9D"
								...
								>{books.bookReviewCount === 0 ?  <LockIcon style={{ color: '#fff' }} />:'Buy Now'}</Button>
								<Dialog open={open1} onClose={handleClose} PaperProps={{
          style: {
           
           
            width: "557px", 
height: "247px",borderRadius:'26px',overflow:'hidden'
            
          }
          }} >
						 <Button onClick={handleClose} style={{ justifyContent: 'flex-end', marginRight: '36px', marginTop: '16px' }}><CloseIcon style={{width:'16px',height:'16px',color:'#1e1e1e'}}/></Button>
						 <DialogContent >
									<DialogTitle style={dialogStyle}>Do you want to add this book to your My List?</DialogTitle>
										<Grid item lg={12} style={{justifyContent:'center',display:'flex'}}>
										<Button style={buttonStyle} onClick={handleClick} >Add to My List</Button>
				...
				<Grid container lg={12} xs={12} sx={{marginBottom:"50px"}} >
  ...
  review && review.bookreview && review.bookreview.trim() !== '' && (
    <Grid container lg={12} key={index} style={{ background: 'white', height: "222px", borderRadius: '20px', border: '1px solid #000', marginBottom: '30px', justifyContent: 'center', alignItems:'center'}}>
								<Grid item lg={11.5}>
								<Typography style={{fontSize:"16px",fontWeight:'400',fontFamily:'Space Grotesk',lineHeight:'24.96px',color:'#000'}}>				{review.userName} </Typography>
									</Grid>
								<Grid item lg={11.5}>
      ...
      <Rating value={ratingObj.rating} readOnly />
								...
								<Grid item lg={11.5}>
        <Typography style={{fontSize:"16px",fontWeight:'400',fontFamily:'Space Grotesk',lineHeight:'24.96px',color:'#3E3E3E'}}>{review.bookreview}</Typography>
								</Grid>
